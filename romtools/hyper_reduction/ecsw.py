...
# TODO scipy loss and enhancement method for testing? Won't work in place of this function because tau can't be specified

class ECSWsolverNNLS(AbstractECSWsolver):
    '''
    Takes linears system G,b, returns mesh indices and weights for ECSW computed via non-negative 
    least squares algorithm from Chapman et al. 2016 DOI: 10.1002/nme.5332. 

    G is a [n_snaps*n_rom by n_dofs] numpy ndarray, where n_snaps is the number of residual 
...
def ECSWFixedTestBasis(ecsw_solver: AbstractECSWsolver, residual_snapshots: np.ndarray, test_basis: np.ndarray, n_vars: int, variable_ordering: str, tau: np.double):
    ...
    tau is the tolerance for the ecsw_solver. It is a small (<<1) positive number
    '''
...
def ECSWVaryingTestBasis(ecsw_solver: AbstractECSWsolver, full_mesh_lhs: np.ndarray, full_mesh_rhs: np.ndarray, tau: np.double):
    '''
    ECSW implementation for a fixed test basis, such as POD-Galerkin projection
