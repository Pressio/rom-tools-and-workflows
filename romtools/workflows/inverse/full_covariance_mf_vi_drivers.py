...
from romtools.workflows.inverse.full_covariance_vi import (
    ...
    full_covariance_log_density if False else log_density,
...
def _optimal_baseline(values, scores):
...
def _component_standard_error(terms):
    if terms.shape[0] <= 1:
...
def _upgrade_mf_state(
    ...
    if n_extra > 0:
        ...
        packed_size = covariance_fom.shape[1]
        mean_extra = np.zeros((0, variational_mean.size))
        covariance_extra = np.zeros((0, packed_size))
    ...
    if entropy_strategy == "joint":
        ...
        raise ValueError("score_function_entropy_strategy must be 'analytic' or 'joint'")
    ...
    if entropy_strategy == "joint":
        high_reference = (
            np.abs(elbo_scaling_factor * np.asarray(state["log_joint_terms_fom"]))[:, None]
    ...
    state = dict(state)
    d = variational_mean.size
    state.update(
        gradient_mean=ordinary_gradient[:d],
        gradient_covariance_svec=ordinary_gradient[d:],
        natural_gradient_mean=natural_gradient[:d],
        natural_gradient_covariance_svec=natural_gradient[d:],
        update_direction_mean=update[:d],
        update_direction_covariance_svec=update[d:],
    ...
    # Compatibility diagnostics retain the mean block and covariance block for
    # componentwise coefficients.  Matrix coefficients remain available in the
    # authoritative mfmc_alpha_packed entry.
    if np.asarray(alpha).ndim == 1:
        state["mfmc_alpha_mean"] = np.asarray(alpha)[:d]
        state["mfmc_alpha_covariance_svec"] = np.asarray(alpha)[d:]
    else:
        state["mfmc_alpha_mean"] = np.asarray(alpha)
        state["mfmc_alpha_covariance_svec"] = np.asarray(alpha)
    ...
    if n_extra > 0:
        ...
        mf_mean_joint = np.mean(state["log_joint_terms_fom"])
    state["elbo"] = elbo_scaling_factor * float(mf_mean_joint) + state["entropy"]
    return state
...
def _evaluate_mf_state(
    *,
    ...
    _, _, variational_log_std, correlation_cholesky = _representation_from_cholesky(
        cholesky
    )
...
def _save_mf_restart(
    ...
    *,
    ...
    dispatcher.np_savez(path, **save_data)


def _append_mf_history(history, mean, cholesky, state, cpu_time, step_size, **kwargs):
    bounded = kwargs["bounded_parameter_handling"]
...
def _save_mf_history(absolute_work_dir, history, state, dispatcher):
    dispatcher.np_savez(
...
def _run_full_covariance_mf_vi(
    *,
    ...
    if fom_sample_size <= 1 or rom_extra_sample_size < 0:
        raise ValueError("fom_sample_size must exceed one and rom_extra_sample_size be nonnegative")
    ...
    if optimization_method == "newton":
        raise NotImplementedError("Full-covariance Newton/Hessian support is out of scope")
    ...
    if optimization_method == "adam":
        ...
        line_search_method, line_config = _resolve_line_search_config(
            line_search_method,
            line_search_config,
            VILegacyLineSearchConfig(step_size_growth_factor=1.05, relaxation_parameter=10.0),
            VIStochasticNonmonotoneLineSearchConfig(step_size_growth_factor=1.05, relaxation_parameter=10.0),
        )
    ...
    nonmonotone_window = getattr(line_config, "line_search_nonmonotone_window", 1)
    ...
    baseline_method = _vi._normalize_baseline_method("loo" if baseline_method is None else baseline_method)
    ...
    correlation_estimator = _mf._normalize_correlation_estimator(correlation_estimator)
    ...
    prior_precision, prior_log_det = _vi._compute_gaussian_log_density_data(prior_covariance)
    ...
    if restart_file is None:
        ...
        step_size = min(float(restart_data["step_size"]), line_config.max_step_size)
    ...
    solver = AdamSolver.from_config(config) if optimization_method == "adam" else SteepestDescentSolver()
    if optimization_method == "adam":
        solver.parameter_dimension = d
    ...
    while iteration < max_iterations and gradient_norm > gradient_norm_tolerance:
        current_fom_size = max(2, int(np.ceil(fom_sample_size * sample_growth**failed_steps)))
        current_rom_extra = 0 if rom_extra_sample_size == 0 else max(
            1, int(np.ceil(rom_extra_sample_size * sample_growth**failed_steps))
        )
        ...
        direction_mean = direction[:d]
        direction_covariance = direction[d:]
        candidate_mean = mean + step_size * direction_mean
        candidate_cholesky, covariance_scale = retract_covariance(
            cholesky,
            packed_direction_to_covariance(direction_covariance, d),
            step_size,
            max_covariance_log_step,
        )
        ...
        ordinary = np.concatenate([state["gradient_mean"], state["gradient_covariance_svec"]])
        effective_direction = np.concatenate([direction_mean, covariance_scale * direction_covariance])
        predicted_slope = float(np.dot(ordinary, effective_direction))

        if optimization_method == "adam":
            accept = True
        elif line_search_method == "legacy":
            allowed_drop = (line_config.relaxation_parameter - 1.0) * abs(state["elbo"])
            accept = candidate["elbo"] >= state["elbo"] - allowed_drop
        else:
            reference = max(accepted_elbo_history[-nonmonotone_window:])
            target = reference + armijo * step_size * max(predicted_slope, 0.0)
            current_se = _mf._compute_state_elbo_standard_error(state, elbo_scaling_factor)
            candidate_se = _mf._compute_state_elbo_standard_error(candidate, elbo_scaling_factor)
            target -= uncertainty * np.sqrt(current_se**2 + candidate_se**2)
        ...
        if accept:
            ...
            failed_steps = 0
            if optimization_method != "adam":
                step_size = min(step_size * line_config.step_size_growth_factor, line_config.max_step_size)
            gradient_norm = _gradient_norm(state)
            cpu_time = time.process_time() - start_cpu
            _append_mf_history(
                history,
            ...
            print(
                ...
                f"Gradient norm: {gradient_norm:.5f}, covariance scale: {covariance_scale:.3f}"
            )
            iteration += 1
            if elbo_relative_tolerance is not None and relative_elbo <= elbo_relative_tolerance:
                elbo_converged = True
                break
        else:
            failed_steps += 1
    ...
    _save_mf_history(absolute_work_dir, history, state, dispatcher)
    final_std = marginal_std_from_cholesky(cholesky)
    if elbo_converged:
        print("ELBO relative-improvement tolerance reached!")
    return mean, final_std, state["parameter_samples_fom"], state["qois_fom"]
...
def run_mf_vi(
    ...
    *,
    ...
    if sample_reuse_config is not None:
        raise NotImplementedError("Sample reuse is not yet defined for full-covariance MF-VI")
    if absolute_vi_directory is not None:
        ...
        warnings.warn("'absolute_vi_directory' is deprecated; use 'absolute_work_dir'.", DeprecationWarning, stacklevel=2)
...
def mf_vi_with_auto_rom(*args, variational_distribution=None, max_covariance_log_step=1.0, **kwargs):
    ...
    # Reuse the existing auto-ROM builder path by temporarily passing the
    # corrected run_mf_vi function through the module-level symbol it calls.
    original = _mf.run_mf_vi
    _mf.run_mf_vi = run_mf_vi
    try:
        kwargs["variational_distribution"] = "full_covariance"
        kwargs["max_covariance_log_step"] = max_covariance_log_step
        return _mf.mf_vi_with_auto_rom(*args, **kwargs)
    finally:
        _mf.run_mf_vi = original
