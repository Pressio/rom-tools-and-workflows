...
def _parallel_argmax_test(ndim, comm):
    ...
    numpy_max_val = global_arr.flatten()[numpy_max_index]

    linalg_max_index = linalg_result[1]
    linalg_max_val = linalg_result[0]

    assert linalg_max_val == numpy_max_val

    rank_with_max = linalg_result[2]

    if rank == rank_with_max:
        assert local_arr.flatten()[linalg_max_index] == linalg_max_val
