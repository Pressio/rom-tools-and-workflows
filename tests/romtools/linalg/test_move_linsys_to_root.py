...
def test_move_linsys_to_rank_zero(numRows):
    np.random.seed(9243)
    comm = MPI.COMM_WORLD
    ...
    for scenario in scenarios:
        A_l, Agold = generate_random_local_and_global_arrays_impl((numRows, 5), comm)
        b_l, bgold = generate_random_local_and_global_arrays_impl((numRows, 1), comm)
        comm.barrier()
        ...
        if scenario == 'rank_0_empty':
            ...
            A2, b2 = move_distributed_linear_system_to_rank_zero(A_l, b_l, comm)
            if myRank == 0:
                rows = [4,5,6,7,8,9,10] if numRows == 11 else [2,3,4]
        ...
        if scenario == 'rank_1_empty':
            ...
            A2, b2 = move_distributed_linear_system_to_rank_zero(A_l, b_l, comm)
            if myRank == 0:
                rows = [0,1,2,3,8,9,10] if numRows == 11 else [0,1,4]
                assert np.allclose(A2, Agold[rows,:])
                assert np.allclose(b2, bgold[rows,0])
        ...
        if scenario == 'rank_2_empty':
            ...
            A2, b2 = move_distributed_linear_system_to_rank_zero(A_l, b_l, comm)
            if myRank == 0:
                rows = [0,1,2,3,4,5,6,7] if numRows == 11 else [0,1,2,3]
                assert np.allclose(A2, Agold[rows,:])
                assert np.allclose(b2, bgold[rows,0])
        ...
        if scenario == 'rank_01_empty':
            ...
            A2, b2 = move_distributed_linear_system_to_rank_zero(A_l, b_l, comm)
            if myRank == 0:
                rows = [8,9,10] if numRows == 11 else [4]
                assert np.allclose(A2, Agold[rows,:])
                assert np.allclose(b2, bgold[rows,0])
        ...
        if scenario == 'rank_02_empty':
            ...
            A2, b2 = move_distributed_linear_system_to_rank_zero(A_l, b_l, comm)
            if myRank == 0:
                rows = [4,5,6,7] if numRows == 11 else [2,3]
                assert np.allclose(A2, Agold[rows,:])
                assert np.allclose(b2, bgold[rows,0])


if __name__ == "__main__":
    test_move_linsys_to_rank_zero(11)
    test_move_linsys_to_rank_zero(5)    