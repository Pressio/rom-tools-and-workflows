...
#### DRAFT: adapted from holdout sampling

class MockFOMQoiModel:
    def __init__(self):
        self.my_qois_ = np.ones([100,3])
        self.counter_ = 1
...
def test_enkf(tmp_path):
    ...
    prior = lambda n_samples, seed: my_parameter_space.generate_samples(n_samples, seed)
    sensor = np.array([2.0,2.1,2.2])
    ## First, test that the routine will terminate if it hits the max iters
    run_enkf_mf(
        QoiModel, RomModelBuilder, sensor, prior, ["u"], my_dir, 1e-3, 1e-3
    )
#  fom_model: QoiModel,
