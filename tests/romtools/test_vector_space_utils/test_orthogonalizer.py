...
@pytest.mark.mpi_skip
def test_euclidean_matrix_weighted_l2_orthogonalizer():
    np.random.seed(1)
    mat_to_orthogonalize_against = np.abs(np.random.normal(size=(10,10)))
    mat_to_orthogonalize_against = mat_to_orthogonalize_against.transpose() @ mat_to_orthogonalize_against
    orthogonalizer = EuclideanMatrixWeightedL2Orthogonalizer(mat_to_orthogonalize_against)
    my_basis = np.random.normal(size=(10, 2))
    my_orthogonalized_basis = orthogonalizer.orthogonalize(my_basis)
    should_be_eye = my_orthogonalized_basis.transpose() @ (mat_to_orthogonalize_against @ my_orthogonalized_basis)
    assert np.allclose(should_be_eye, np.eye(2)), should_be_eye
...
if __name__ == "__main__":
    ...
    test_euclidean_matrix_weighted_l2_orthogonalizer()
