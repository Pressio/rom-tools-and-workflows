...
def collect_history(method_dir: Path, truth: np.ndarray, reuse_enabled: bool) -> dict:
    ...
    with np.load(history_path, allow_pickle=True) as history:
        ...
        covariance = np.asarray(history["vi_history_variational_covariance"], dtype=float)
    ...
    while True:
        ...
        with np.load(restart_path, allow_pickle=True) as data:
            ...
            relative_mse.append(
                float(data["mean_relative_mse"]) if "mean_relative_mse" in data else np.nan
            )
            reuse.append(
                {
                    "used": _read_restart_value(data, "sample_reuse_used"),
                    "ess": _read_restart_value(data, "sample_reuse_ess"),
                    "archive_samples": _read_restart_value(data, "sample_reuse_archive_samples"),
                    "archive_batches": _read_restart_value(data, "sample_reuse_archive_batches"),
                    "refresh_reason": _read_restart_value(data, "sample_reuse_refresh_reason"),
                }
            )
        iteration += 1
        ...
        for diagnostic, cumulative_work in zip(reuse, work):
            if diagnostic["used"] is None:
                ...
                diagnostic["refresh_reason"] = (
                    "inferred_reuse_no_new_fom" if used else "inferred_refresh_new_fom"
                )
            previous_work = int(cumulative_work)

    parameter_error = np.sqrt(np.mean(((means - truth[None, :]) / truth[None, :]) ** 2, axis=1))
    std = np.sqrt(np.maximum(np.diagonal(covariance, axis1=1, axis2=2), 0.0))
    return {
...
def aggregate_runs(results: list[dict], method_order: list[str]) -> dict:
    ...
    for method in method_order:
        ...
        if not runs:
            continue
        max_work = max(int(run["history"]["cumulative_fom_evaluations"][-1]) for run in runs)
        grid = np.arange(1, max_work + 1, dtype=float)
        entry = {"label": runs[0]["label"], "fom_grid": grid, "num_seeds": len(runs)}
        for key in ("parameter_relative_error", "elbo", "mean_relative_mse"):
            values = np.vstack([_interpolate(run["history"], grid, key) for run in runs])
            entry[key] = {
...
def write_plots(aggregate: dict, output_dir: Path, suffix: str = "ablation") -> None:
    ...
    for metric, ylabel in (
        ...
        figure.tight_layout()
        figure.savefig(output_dir / f"h2_air_flame_vi_{suffix}_{metric}.png", dpi=180)
